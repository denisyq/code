...
int main(int argc, char *argv[])
{
    ...
    CHK2(client_fd,socket(AF_INET, SOCK_STREAM, 0));					//socket
	...
	while(continue_to_work){
		...
		CHK2(epoll_events_cnt,epoll_wait(epfd,events,2,EPOLL_RUN_TIMEOUT));
		for(int i=0;i<epoll_events_cnt;i++){
			if(events[i].data.fd == client_fd){//server msg
